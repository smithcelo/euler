function transformarTrianguloEmMatriz(input) {
    let linhas = input.trim().split('\n');
    let matriz = linhas.map(linha => linha.split(' ').map(Number));
    return matriz;
}

function maxTotalFromTopToBottom(triangulo) {
...
let matriz = transformarTrianguloEmMatriz(input);
let maxTotal = maxTotalFromTopToBottom(matriz);
